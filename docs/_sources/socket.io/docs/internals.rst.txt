...
-  the content is then decoded by the ``engine.io-parser`` on the
   client-side
...
Once all the buffers of the existing transport (XHR polling) are
flushed, an upgrade gets tested on the side by sending a probe.
...
-  a “ping” packet is sent by the client in a WebSocket frame, encoded
   as ``2probe`` by the ``engine.io-parser``, with ``2`` being the
   “ping” message type.

-  the server responds with a “pong” packet, encoded as ``3probe``, with
   ``3`` being the “pong” message type.

-  upon receiving the “pong” packet, the upgrade is considered complete
   and all following messages go through the new transport.
